...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
**FIT — Freelance Interconnect** REST API. Cameroon's freelancing marketplace: clients publish jobs or order services, freelancers propose and deliver, payments are escrowed in FCFA via Mobile Money and released on client approval.

### Authentication
Register or log in to receive a **Sanctum bearer token**, then send it as `Authorization: Bearer {token}`. Endpoints marked with a padlock require it; the rest are public.

### Conventions
- Base path: `/api/v1`
- Money is in **XAF** unless stated otherwise; connects purchases also support USD.
- Lists are paginated (`page`, `per_page`) and return Laravel's standard `data` / `links` / `meta` envelope.
- Validation errors return **422** with `{ message, errors }`; missing/invalid tokens return **401**; role or ownership violations return **403**.

### Payments sandbox
Payment endpoints create a *pending* payment intent. In development, settle it with `POST /payments/{reference}/confirm-sandbox`, or simulate the aggregator with `POST /webhooks/payments`.
MD,
    ],

    'ui' => [
        'title' => 'FIT API Documentation',
    ],
